...
NAN_METHOD(scrypt) {
   ...
   Local<Number> numn = args[1]->ToNumber(Isolate::GetCurrent()->GetCurrentContext()).ToLocalChecked();
   unsigned int nValue = numn->Value();
   Local<Number> numr = args[2]->ToNumber(Isolate::GetCurrent()->GetCurrentContext()).ToLocalChecked();
...
NAN_METHOD(scryptn) {
   ...
   Local<Number> num = args[1]->ToNumber(Isolate::GetCurrent()->GetCurrentContext()).ToLocalChecked();
...
NAN_METHOD(scryptjane) {
    ...
    Local<Number> num2 = args[2]->ToNumber(Isolate::GetCurrent()->GetCurrentContext()).ToLocalChecked();
    int nChainStartTime = num2->Value();

    Local<Number> num3 = args[3]->ToNumber(Isolate::GetCurrent()->GetCurrentContext()).ToLocalChecked();
    int nMin = num3->Value();

    Local<Number> num4 = args[4]->ToNumber(Isolate::GetCurrent()->GetCurrentContext()).ToLocalChecked();
    int nMax = num4->Value();
